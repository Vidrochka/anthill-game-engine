...
impl Container {
    pub fn build_singletone<TType>(&mut self, injector: &mut Injector) -> Arc<Mutex<TType>> where TType: 'static {
        println!("[container::build_singletone] {:?}, {:?}", type_name::<TType>(), TypeId::of::<TType>());
        if let Some(instance) = self.instance.take() {
            match instance.downcast::<Arc<Mutex<TType>>>() {
                ...
                Err(val) => {
                    ...
                    panic!("[container::build_singletone::select] invalid di cast type [{}] to [{}]", type_name_of_val(&self.instance), type_name::<Arc<Mutex<TType>>>())
                },
            }
        } else {
    ...
    pub fn build_new_instance<TType>(&self, injector: &mut Injector) -> TType where TType: 'static {
        println!("[container::build_new_instance] {:?}, {:?}", type_name::<TType>(), TypeId::of::<TType>());
        if let Some(constructor) = &self.constructor {
            let constructed = (constructor)(injector);
